...
// ctxWithFilter creates a context with a PipelineContext that has the given filter set.
func ctxWithFilter(filter domain.SearchFilter) context.Context {
	pc := pipeline.NewPipelineContext("")
	pc.SetFilter(filter)
	return pipeline.WithPipelineContext(context.Background(), pc)
}

func TestMergeAndFilter_FilterNonBookable(t *testing.T) {
	// Matches PHP testFinishCookOldFormatWhenOptionUnavailable behavior
	stage := NewMergeAndFilterStage()
	in := HydratedResults{
	...
	out, err := stage.Execute(ctxWithFilter(domain.SearchFilter{WithNonBookable: false}), in)
	if err != nil {
...
func TestMergeAndFilter_FilterInvalidPrice(t *testing.T) {
	stage := NewMergeAndFilterStage()
	in := HydratedResults{
		Trips: []domain.TripResult{
	...
	out, err := stage.Execute(ctxWithFilter(domain.SearchFilter{}), in)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestMergeAndFilter_PresentIntegrations(t *testing.T) {
...
func TestMergeAndFilter_MergeDuplicates(t *testing.T) {
	stage := NewMergeAndFilterStage()
...
func TestMergeAndFilter_DedupTravelOptions(t *testing.T) {
	stage := NewMergeAndFilterStage()
...
func TestMergeAndFilter_RecheckKeys(t *testing.T) {
	stage := NewMergeAndFilterStage()
...
func TestMergeAndFilter_EmptyInput(t *testing.T) {
	stage := NewMergeAndFilterStage()
